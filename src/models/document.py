...
class Paragraph:
    ...
    level: int | None = None             # 缩进/大纲级别 (0=无缩进, 1=一级缩进...)
...
class PageElement:
    ...
    # 形态标签
    shape_name: str | None = None        # PPTX shape name
...
class Page:
    ...
    def flattened_elements(self) -> list:
        ...
        for elem in self.elements:
            if elem.type is None:
                logger.warning("Page %d: 跳过 type=None 的异常元素 (%s)", self.index, elem.shape_name or "unknown")
                continue
    ...
    def tables(self) -> list[PageElement]:
        ...
        return [e for e in self.flattened_elements
                if e.type == "table"]
